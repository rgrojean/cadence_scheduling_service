...
import { composeDisplayName, getPatient } from "../identity.js";
...
export async function runReminders(log: (msg: string) => void = console.log): Promise<number> {
  ...
  for (const appt of appointments) {
    try {
      ...
      const message = `Riverbend Health reminder: appointment for ${composeDisplayName(patient)} on ${new Date(appt.starts_at).toISOString()}. Reply HELP for help.`;
